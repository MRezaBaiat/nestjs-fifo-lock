...
describe('LockService', () => {
    ...
    it('throws if queue entry disappears before acquisition', async () => {
        ...
        await expect(service.auto('A', async () => { })).rejects.toThrow(/lock request got deleted/);
    });
    it('preserves FIFO ordering for multiple waiters', async () => {
        const order = [];
        await Promise.all([1, 2, 3].map((i) => service.auto('A', async () => {
            order.push(i);
            await sleep(10);
        })));
        expect(order).toEqual([1, 2, 3]);
        order.length = 0;
        const tasks = [[1], [1, 2], [3]].map((i) => service.auto(i.map(n => String(n)), async () => {
            order.push(i);
            await sleep(100);
        }));
        await sleep(120);
